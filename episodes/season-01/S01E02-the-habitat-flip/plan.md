# S01E02 — "The Habitat Flip"

## Location
- **Name:** Butcher Jones Recreation Site / Saguaro Lake, Fort McDowell, AZ
- **Country:** US
- **Region/state:** Arizona
- **Coordinates:** 33.5767, -111.5147
- **Distance from home base:** ~50 mi
- **Trail/position:** Northwest parking lot near the wash
- **Fee:** Tonto Pass required ($8/day or annual)
- **Opens:** Gate opens at 7AM — verify early access options

## Purpose
Habitat contrast — riparian/water vs. desert. Sunrise over open water. This episode demonstrates that the scoring system works across different environments.

## Key content beats
- Contrast this habitat with E01's desert setting
- Open water horizon should produce highest sky scoring ceiling
- Water birds vs. desert birds — the "habitat flip"
- Gate timing is a real logistical challenge — show it honestly
- Compare Zora Score breakdown to E01 — different strengths

## Sunrise note
Open water horizon — highest scoring ceiling in the Phoenix region. If the sky cooperates, this could top the leaderboard.

## Travel details
- **Distance (mi):** ~50
- **Off-pavement (mi):** 0
- **High-clearance required:** no
- **Alt transport:** no
- **International:** no
- **Elevation gain (ft):** minimal

## Discovery targets
| Species/feature | Type | Rarity tier | Points | First unlock? |
|-----------------|------|-------------|--------|---------------|
| Great Blue Heron | Wildlife | Uncommon | 15-20 | Yes |
| Great Egret | Wildlife | Uncommon | 15-20 | Yes |
| Osprey | Wildlife | Rare | 25-35 | Yes |
| Vermilion Flycatcher | Wildlife | Rare | 25-35 | Yes |
| Bald Eagle (winter) | Wildlife | Very rare | 40-50 | Yes |
| Mule Deer | Wildlife | Uncommon | 15-20 | Yes |

## Shot list
- [ ] Pre-dawn drive — longer route, show the distance commitment
- [ ] Gate approach — will it be open?
- [ ] Establishing wide — lake at dawn
- [ ] Water bird encounters
- [ ] Locked timelapse — sunrise over Saguaro Lake
- [ ] On-camera scoring — both Eos Index and Zora Score, leaderboard comparison to E01
- [ ] GoPro POV running throughout

## Logistics
- **Sunrise time:** _(fill before shoot)_
- **Target arrival:** _(just before 7AM — gate dependent)_
- **Minutes before sunrise:** _(fill on location)_
- **Weather forecast:** _(fill night before)_
- **Moon phase:** _(fill night before)_
- **eBird recent sightings:** _(check night before)_

---

## Eos Index — sunrise quality (fill after filming)

### Sky (max 50)
| Component | Score | Notes |
|-----------|-------|-------|
| Color intensity (max 20) | | |
| Cloud engagement (max 15) | | |
| Horizon definition (max 15) | | |
| **Sky total** | **/50** | |

### Setting (max 30)
| Component | Score | Notes |
|-----------|-------|-------|
| Foreground composition (max 15) | | |
| Location uniqueness (max 15) | | |
| **Setting total** | **/30** | |

### Conditions (max 20)
| Component | Score | Notes |
|-----------|-------|-------|
| Effort to reach (max 10) | | |
| Weather/environmental challenge (max 10) | | |
| **Conditions total** | **/20** | |

### Eos Index: __/100

---

## Zora Score — full episode (fill after filming)

| Component | Points | Notes |
|-----------|--------|-------|
| Eos Index | /100 | From above |
| Travel distance | /5 | ~50 mi → 3 pts |
| Travel difficulty bonus | | No bonuses expected |
| Elevation gain | /4 | Minimal → 0 pts |
| Pre-dawn arrival | /2 | Gate may limit this |
| Weather adversity | /3 | |
| Discovery points | | All first unlocks — rich discovery potential |
| Streak bonus | | +0 (need 4 consecutive for first bonus) |
| **Zora Score total** | | |

---

## Discoveries (fill after filming)
| Name | Type | Rarity | Points | First unlock? | Photo? | Fun fact |
|------|------|--------|--------|---------------|--------|----------|
| | | | | | | |

## Field notes
_What happened, what surprised you, what would you do differently._
