# S01E03 — "The Time Capsule"

## Location
- **Name:** Hieroglyphic Trailhead, Gold Canyon, AZ
- **Country:** US
- **Region/state:** Arizona
- **Coordinates:** 33.3901, -111.4233
- **Distance from home base:** ~25 mi
- **Trail/position:** 2.8 mi out-and-back to Hohokam petroglyph panel
- **Fee:** Free (limited parking — arrive very early or weekday)
- **Opens:** 5AM — best pre-dawn access of the three

## Purpose
Deep edutainment — 1,500-year-old Hohokam petroglyphs, canyon framing. This episode introduces the cultural/historical discovery type and shows the show has depth beyond pretty skies. Best episode to demonstrate how the Zora Score rewards effort beyond just sunrise quality.

## Key content beats
- Explain Hohokam history and why these carvings exist here
- Tinajas (natural water basins) — what they are, why they matter
- Canyon position delays disk emergence but the canyon wall light show is exceptional
- Longest hike of the first three — effort score and elevation gain should be highest
- First cultural/historical discovery — introduce the Human History category
- This episode should demonstrate the Zora Score diverging from the Eos Index

## Sunrise note
Canyon position delays disk emergence but canyon wall light show is exceptional. Different kind of sunrise — reflected light, not direct.

## Travel details
- **Distance (mi):** ~25
- **Off-pavement (mi):** 0
- **High-clearance required:** no
- **Alt transport:** no
- **International:** no
- **Elevation gain (ft):** ~500 (2.8 mi trail with moderate gain)

## Discovery targets
| Species/feature | Type | Rarity tier | Points | First unlock? |
|-----------------|------|-------------|--------|---------------|
| Rock Wren | Wildlife | Uncommon | 15-20 | Yes |
| Roadrunner | Wildlife | Uncommon | 15-20 | Yes |
| Desert Tortoise | Wildlife | Very rare | 40-50 | Yes |
| Chuckwalla | Wildlife | Very rare | 40-50 | Yes |
| Canyon Towhee | Wildlife | Uncommon | 15-20 | Yes |
| Cooper's Hawk | Wildlife | Rare | 25-35 | Yes |
| Hohokam petroglyphs | Cultural/Historical | Significant | 15-20 | Yes |
| Tinajas | Geographic | — | TBD | Yes |

## Shot list
- [ ] Pre-dawn trailhead — 5AM start, headlamps
- [ ] Trail approach — 1.4 mi in, show the effort
- [ ] Petroglyph panel — respectful, educational
- [ ] Tinajas — explain what they are
- [ ] Wildlife encounters
- [ ] Locked timelapse — canyon wall light show
- [ ] On-camera scoring — both systems, leaderboard update
- [ ] GoPro POV running throughout

## Logistics
- **Sunrise time:** _(fill before shoot)_
- **Target arrival:** _(5AM trailhead open)_
- **Minutes before sunrise:** _(fill on location — should be 60+ for pre-dawn bonus)_
- **Weather forecast:** _(fill night before)_
- **Moon phase:** _(fill night before)_
- **eBird recent sightings:** _(check night before)_

---

## Eos Index — sunrise quality (fill after filming)

### Sky (max 50)
| Component | Score | Notes |
|-----------|-------|-------|
| Color intensity (max 20) | | |
| Cloud engagement (max 15) | | |
| Horizon definition (max 15) | | Canyon may limit this |
| **Sky total** | **/50** | |

### Setting (max 30)
| Component | Score | Notes |
|-----------|-------|-------|
| Foreground composition (max 15) | | Petroglyphs, canyon walls, tinajas |
| Location uniqueness (max 15) | | 1,500-year-old archaeological site |
| **Setting total** | **/30** | |

### Conditions (max 20)
| Component | Score | Notes |
|-----------|-------|-------|
| Effort to reach (max 10) | | 2.8 mi hike, pre-dawn start |
| Weather/environmental challenge (max 10) | | |
| **Conditions total** | **/20** | |

### Eos Index: __/100

---

## Zora Score — full episode (fill after filming)

| Component | Points | Notes |
|-----------|--------|-------|
| Eos Index | /100 | From above |
| Travel distance | /5 | ~25 mi → 2 pts |
| Travel difficulty bonus | | No bonuses expected |
| Elevation gain | /4 | ~500 ft → 1 pt |
| Pre-dawn arrival | /2 | 5AM open, sunrise ~6:30 → 2 pts (60+ min) |
| Weather adversity | /3 | |
| Discovery points | | Richest discovery episode — wildlife + cultural |
| Streak bonus | | +0 (need 4 consecutive for first bonus) |
| **Zora Score total** | | |

---

## Discoveries (fill after filming)
| Name | Type | Rarity | Points | First unlock? | Photo? | Fun fact |
|------|------|--------|--------|---------------|--------|----------|
| | | | | | | |

## Field notes
_What happened, what surprised you, what would you do differently._
