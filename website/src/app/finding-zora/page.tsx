...
import { isAired, formatAirsDate, formatAirsDateShort } from "@/lib/airing";

export const metadata: Metadata = { title: "finding zora" };
export const dynamic = "force-dynamic";

export default async function FindingZoraHub() {
  const episodes = await getEpisodes();
  // Featured "Latest expedition" + medallion progress reflect AIRED episodes
  // only — otherwise an upcoming entry would headline and inflate the level.
  const airedEpisodes = episodes.filter((e) => isAired(e.publish_date));
  const latest = airedEpisodes[0] ?? null;
  const totalExpeditions = airedEpisodes.length;
    ...
    <div>
      ...
      <LivingCover>
        <div className="max-w-[600px]">
          ...
          <h1
            className="font-display-ornate text-zora-amber font-bold leading-none mb-1 mt-5"
            style={{
      ...
      <div className="max-w-[780px] mx-auto px-8 pb-16">
        ...
        {latest && (
          <>
            ...
            <Link
              href={`/finding-zora/episodes/s${String(latest.season).padStart(2, "0")}e${String(latest.episode_number).padStart(2, "0")}`}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            { href: "/finding-zora/rules", label: "how it works", desc: "Scoring, leveling, streaks, discoveries" },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="bg-pre-dawn-mid border border-rule rounded-md p-4 hover:border-zora-amber/40 transition-colors"
            >
              <p className="font-display text-sm text-dawn-mist">{item.label}</p>
              <p className="text-xs text-mist-dim mt-1">{item.desc}</p>
            </Link>
          ))}
        </div>
      </div>
      ...
      {grouped.length > 0 ? (
        <div className="bg-pre-dawn-mid border border-rule rounded-md">
          ...
          {grouped.length > limit && (
            <Link
              href="/finding-zora/discovery-log"
